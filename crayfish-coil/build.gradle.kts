...
mavenPublishing {
  ...
  pom {
    name.set(artifactId)
    description.set(
      "Coil 3 integration for Crayfish: re-apply a stored crop while Coil loads the original, " +
        "with no second file and no second decode.",
    )
  }
}
...
kotlin {
  android {
    namespace = "com.github.skydoves.crayfish.coil"
  ...
  sourceSets {
    commonMain.dependencies {
      // `api`, not `implementation`: a caller writing `CropTransformation(...)` into a Coil
 ...
 * platform actuals here are the only code in the repo that reaches into Coil's own bitmap types,
 * and nothing else would notice them rotting.
 */
